...
template<class KeyT, class ValueT>
HashMap<KeyT, ValueT>::HashMap () : HashMap (vector<KeyT> (), vector<ValueT>
    ())
{}
...
template<class KeyT, class ValueT>
unsigned int HashMap<KeyT, ValueT>::hash (KeyT key) const
{
...
template<class KeyT, class ValueT>
void HashMap<KeyT, ValueT>::re_hash ()
{
...
template<class KeyT, class ValueT>
HashMap<KeyT, ValueT>::HashMap (const HashMap<KeyT, ValueT> &map)
{
  _size = map._size;
...
template<class KeyT, class ValueT>
void HashMap<KeyT, ValueT>::adjust_capacity_to_balance_load_factor ()
{
  if (_size == 0)
  {
    _capacity = 1;
  ...
  {
    while (get_load_factor () < lower_load_factor && _capacity > MIN_CAPACITY)
    {
      _capacity /= 2;
    }
    return;
